...
------------ 22000-CALCULA-FECHA (SE RESTAN 3 MESES A LA FECHA EN CURSO) ----------
   SET @IP_FECHA_3M = DATEADD(MONTH, -3,GETDATE())
------------------------------------------------------------------------------------
    ...
    --********************************
            ...
            ISNULL(CAST(T071_NUM_OPERATION as varchar(9)),'000000000') + '|@' + -- 9
                ...
                LEFT JOIN  MAZP.BGDT606 with (nolock) ON
                    ...
                    WHERE T071_ACC = @IP_COD_PROD+@IP_NUM_ACC AND 
                        T071_CEN_REG = @IP_BRN_OPEN AND 
                        T071_ENT = @IP_ENT_IN AND
                        T071_DAT_OPERATION > = @IP_FECHA AND --VAR ENTRADA
                        --T071_DAT_OPERATION + CAST(T071_NUM_OPERATION as varchar(9)) COLLATE SQL_Latin1_General_CP1_CI_AS < @IP_ULLAVE AND --FIJO
                T071_DAT_OPERATION + STR(T071_NUM_OPERATION) < @IP_ULLAVE AND --FIJO
                T071_FLG_ANN = 'N' --FIJO
            ...
            BEGIN
                ...
                LEFT JOIN MAZP.BGDT606 with (nolock) ON
                    ...
                    WHERE T071_ACC = @IP_COD_PROD+@IP_NUM_ACC AND 
                            ...
                            T071_ENT = @IP_ENT_IN AND
                        T071_DAT_OPERATION > = @IP_FECHA AND --VAR ENTRADA
                        T071_DAT_OPERATION + STR(T071_NUM_OPERATION) COLLATE SQL_Latin1_General_CP1_CI_AS < @IP_ULLAVE AND --FIJO
                        T071_FLG_ANN = 'N' --FIJO
                    ORDER BY T071_DAT_OPERATION DESC , T071_NUM_OPERATION DESC
                FOR READ ONLY
